...
sub_directories="${4:-traefik,lets-encrypt}"
owner="${5:-$SUDO_USER}"
group="${6:-$SUDO_USER}"
...
# Ensure that the acme.json has the correct permissions required by Traefik.
chmod 600 "$DOCKER_VOLUME"/lets-encrypt/acme.json
