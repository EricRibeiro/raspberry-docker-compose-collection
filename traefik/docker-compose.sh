...
# and prepares the Docker volume directory with the required configuration files and dependencies.
...
function main {
  ...
  if [ "$command" == "up" ]; then
    ...
    local -r skip_acme="$2"
    docker_compose_up "$DOCKER_VOLUME" "$skip_acme"
